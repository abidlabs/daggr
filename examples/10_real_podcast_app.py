"""
🎙️ Document to Podcast Generator - Daggr Demo
==============================================
Transform any document or article into an engaging podcast conversation!

This workflow:
1. Extracts and summarizes content from a URL or document
2. Generates a natural dialogue script between two hosts
3. Creates voice audio for each speaker (scatter)
4. Combines all audio into a final podcast episode

Perfect for: content repurposing, accessibility, learning on the go.

Run with: daggr daggr_podcast_demo.py
"""
...
def generate_dialogue(
    ...
    """
    # Determine number of exchanges based on length
    exchanges = {
        "Short (2-3 min)": 6,
        "Medium (5-7 min)": 12,
        "Long (10+ min)": 20
    ...
    # In production, this would be LLM-generated based on content
    dialogue = []
    ...
    for line in dialogue[:6]:  # Show first 6 lines
...
voice_generator = FnNode(
    ...
    inputs={
        "dialogue": dialogue_generator.dialogue,  # Pass the full list, NOT .each
    },
    outputs={
...
final_podcast = FnNode(
    fn=combine_podcast,
    inputs={
        "audio_files": voice_generator.audio_files,  # Direct reference, no .all()
